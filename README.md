...
- High-Level Metrics: View the total records processed, the number of blank rows removed , and rows dropped due to logic errors.
+1
...
- Fallback Export: Change `OUTPUT_CSV` and `METRICS.CSV` to set the filenames used if the script needs to save data locally if SQL is not connected.
